...
img_gurultuazalt = cv2.bilateralFilter(img_gray, 9, 40, 40)
plt.imshow(img_gurultuazalt, cmap="gray")
plt.show()


img_hstgrm = cv2.equalizeHist(img_gurultuazalt)
plt.imshow(img_hstgrm, cmap="gray")
plt.show()


kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
morfolojikresim = cv2.morphologyEx(img_hstgrm, cv2.MORPH_OPEN, kernel, iterations=15)
plt.imshow(morfolojikresim, cmap="gray")
plt.show()


gcikarilmisresim = cv2.subtract(img_hstgrm, morfolojikresim)
plt.imshow(gcikarilmisresim, cmap="gray")
plt.show()    


ret, goruntuesikle = cv2.threshold(gcikarilmisresim, 0, 255, cv2.THRESH_OTSU)
plt.imshow(goruntuesikle, cmap="gray")
plt.show()
...
#Jonh f Canny
kenarlik = cv2.Canny(goruntuesikle,low,high)
